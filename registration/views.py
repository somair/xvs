...
from profiles.forms import VolunteerProfileForm

from profiles.models import HowDidYouHear, Faculty, ServiceLevelAgreement, Department
...
def register(request, backend, success_url=None, form_class=None,
    ...
    **Template:**
    ...
    # As well as the registration form we pass a volunteer profile form to the template.
    vp_form = VolunteerProfileForm()

    if request.method == 'POST':
        form = form_class(data=request.POST, files=request.FILES)

        # If this is a volunteer, also populate the vp_form so that it can be checked for validity
        is_volunteer = request.POST['reg_type'] == "volunteer"
        if is_volunteer:
            vp_form = VolunteerProfileForm(data=request.POST)
            
        if form.is_valid() and form.errors == {} and ((not is_volunteer) or vp_form.is_valid()):
            new_user = backend.register(request, **form.cleaned_data)

            if is_volunteer:
                # Now build and save the volunteer profile against the base profile.
                vp = vp_form.save(commit=False)
                vp.profile = new_user.get_profile()
                vp.save()
            else:
                if settings.FEATURE_SERVICE_LEVEL_AGREEMENTS:
                    # Save the SLA information.
                    slas = ServiceLevelAgreement.objects.all()
                    profile = new_user.get_profile()
                    for sla in slas:
                        slaid = "sla_%d" % sla.id
                        if slaid in request.POST and request.POST[slaid] == "yes":
                            # Create SLA against profile.
                            profile.slas.add(sla)
                    profile.save()

                if settings.FEATURE_DEPARTMENTS:
                    department = Department.objects.get(pk=request.POST['dep'])
                    profile = new_user.get_profile()
    ...
    return render_to_response(template_name, {
        'form': form, 
        'vpform': vp_form, 
        'hows': HowDidYouHear.objects.all(),
